...
import type { A2LiveSignal, A2GateCriterion, ProfileRecommendation } from "@/lib/engines/atelier2";
import { A2_PROFILE_LABELS } from "@/types/atelier2";

type Props = {
  projectId: string;
  signals: A2LiveSignal[];
  gate: A2GateCriterion[];
  gateVerdict: "NOT_READY" | "READY" | "OVERRIDE" | null;
  profile: ProfileRecommendation;
};

const LEVEL_ICON = { INFO: Info, WARNING: AlertTriangle, BLOCKER: ShieldAlert } as const;

const LEVEL_BG = {
  INFO: "border-sky-500/30 bg-sky-50 text-sky-900 dark:bg-sky-950/40 dark:text-sky-200",
  WARNING: "border-amber-500/40 bg-amber-50 text-amber-900 dark:bg-amber-950/40 dark:text-amber-200",
  BLOCKER: "border-rose-500/40 bg-rose-50 text-rose-900 dark:bg-rose-950/40 dark:text-rose-200",
} as const;

export function Atelier2LivePanel({ projectId, signals, gate, gateVerdict, profile }: Props) {
  const metCount = gate.filter((c) => c.met).length;

  return (
    <aside className="space-y-5 text-sm">
      {/* Profil recommandé — la vraie valeur ajoutée de l'atelier 2 */}
      <section className="space-y-2">
        ...
        <div className="rounded-md border border-foreground/15 bg-muted/30 px-3 py-2.5">
          <div className="text-sm font-semibold">{A2_PROFILE_LABELS[profile.profile]}</div>
          ...
          <ul className="space-y-2">
            {signals.map((sig) => {
              ...
              return (
                <li
                  key={sig.id}
                  className={cn("rounded-md border px-3 py-2 text-xs leading-snug", LEVEL_BG[sig.level])}
                >
                  <div className="flex items-start gap-1.5">
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      {sig.sectionHint ? (
                        <Link
                          href={`/projects/${projectId}/atelier/2/${sig.sectionHint}`}
      ...
      <section className="space-y-2">
        <header className="flex items-center justify-between gap-2">
          ...
          <Badge
            variant={gateVerdict === "READY" || gateVerdict === "OVERRIDE" || metCount === gate.length ? "default" : "outline"}
          >
        ...
        <Link
          href={`/projects/${projectId}/atelier/2/gate`}
