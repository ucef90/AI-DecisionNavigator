...
import { computeA2Gate, loadAtelier2Snapshot } from "@/lib/engines/atelier2";

export default async function A2GatePage(props: PageProps<"/projects/[id]/atelier/2/gate">) {
  const { id } = await props.params;
  const snap = await loadAtelier2Snapshot(id);
  if (!snap) notFound();
  const criteria = computeA2Gate(snap);
  ...
  return (
    <SectionShell
      ...
      title="Gate atelier 3"
      livrableRef="Gate de sortie atelier 2"
      intent="6 critères go/no-go pour passer à l'atelier de cadrage IA (atelier 3)."
      pourquoi={["Sans matrice complète, le scoring atelier 4 sera flou.", "Le gate force l'explicitation de la décision.", "OVERRIDE possible avec justification."]}
      cherche={["6/6 critères au vert.", "Validations humaines et synthèse rédigée."]}
    >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold">
                {verdict === "READY" ? "Prêt pour l'atelier 3" : verdict === "OVERRIDE" ? "Passage forcé" : "Pas encore prêt"}
              </div>
