...
import { computeA3Gate, loadAtelier3Snapshot } from "@/lib/engines/atelier3";

export default async function A3GatePage(props: PageProps<"/projects/[id]/atelier/3/gate">) {
  const { id } = await props.params;
  const snap = await loadAtelier3Snapshot(id);
  if (!snap) notFound();
  const criteria = computeA3Gate(snap);
  ...
  return (
    <SectionShell
      ...
      title="Gate atelier 4 (scoring)"
      livrableRef="Gate de sortie atelier 3"
      intent="7 critères go/no-go pour passer au scoring atelier 4."
      pourquoi={["Sans cadrage propre, scoring biaisé.", "Le gate force la complétude."]}
      cherche={["Couverture ≥ 70%.", "Réglo + maturité + faisabilité OK.", "Synthèse rédigée."]}
    >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold">
                {verdict === "READY" ? "Prêt pour l'atelier 4" : verdict === "OVERRIDE" ? "Passage forcé" : "Pas encore prêt"}
              </div>
