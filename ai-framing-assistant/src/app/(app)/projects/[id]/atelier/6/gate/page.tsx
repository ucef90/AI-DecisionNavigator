...
import { computeA6Gate, loadAtelier6Snapshot } from "@/lib/engines/atelier6";

export default async function A6GatePage(props: PageProps<"/projects/[id]/atelier/6/gate">) {
  const { id } = await props.params;
  const snap = await loadAtelier6Snapshot(id);
  if (!snap) notFound();
  const criteria = computeA6Gate(snap);
  ...
  return (
    <SectionShell
      ...
      title="Gate atelier 7 (industrialisation)"
      livrableRef="Gate de sortie atelier 6"
      intent="8 critères pour passer à l'atelier final (architecture cible, roadmap, décision)."
      pourquoi={["Sans gouvernance solide, industrialisation = échec.", "Validations + sécurité + conformité + monitoring + incidents : indispensables."]}
      cherche={["8/8 critères au vert."]}
    >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-semibold">
                {verdict === "READY" ? "Prêt pour l'atelier 7" : verdict === "OVERRIDE" ? "Passage forcé" : "Pas encore prêt"}
              </div>
