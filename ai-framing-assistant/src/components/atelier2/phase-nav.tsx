...
type Props = {
  projectId: string;
  currentSectionId: Atelier2SectionId | null;
...
export function Atelier2PhaseNav({
  projectId,
  currentSectionId,
  sectionProgress,
  phaseProgress,
  ...
  return (
    <nav className="space-y-4 text-sm">
      <div>
        <Link
          href={`/projects/${projectId}/atelier/2`}
          className={cn(
            ...
            <ul className="space-y-0.5">
              {phase.sections.map((section) => {
                ...
                return (
                  <li key={section.id}>
                    <Link
                      href={`/projects/${projectId}/atelier/2/${section.id}`}
                      className={cn(
